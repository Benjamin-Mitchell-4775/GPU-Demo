...
SIM_STEPS        = 5250         # hard stop — live version ends exactly here
...
def init_live_plots(err_ref):
    plt.ion()
    fig, axes = plt.subplots(2, 2, figsize=(14, 8))
    fig.suptitle("LSTM Entry Guidance — Live Simulation (5250 Steps)", fontsize=13)
    ...
    for ax, title, ylabel in [
        ...
        (ax_err,  "Position Error (X/Y/Z)",  "Error [m]"),
    ...
    # Bank angle — Apollo full reference + LSTM overlay
    ...
    # Position error — Apollo full reference (all 3 axes), faded static
    t_ref = np.arange(len(err_ref))
    ax_err.plot(t_ref, err_ref[:, 0] * 1e-3, color="tab:red",    alpha=0.25, linewidth=1.0, linestyle="--", label="Apollo dis_x")
    ax_err.plot(t_ref, err_ref[:, 1] * 1e-3, color="tab:green",  alpha=0.25, linewidth=1.0, linestyle="--", label="Apollo dis_y")
    ax_err.plot(t_ref, err_ref[:, 2] * 1e-3, color="tab:blue",   alpha=0.25, linewidth=1.0, linestyle="--", label="Apollo dis_z")

    # LSTM live overlay lines
    line_err_x, = ax_err.plot([], [], color="tab:red",   linewidth=1.4, label="LSTM dis_x")
    line_err_y, = ax_err.plot([], [], color="tab:green", linewidth=1.4, label="LSTM dis_y")
    line_err_z, = ax_err.plot([], [], color="tab:blue",  linewidth=1.4, label="LSTM dis_z")
    ax_err.legend(fontsize=7, ncol=2)

    # Keep the old scalar error line for the magnitude panel (unused panel now repurposed)
    line_err,   = ax_err.plot([], [])   # hidden — kept so return signature stays compatible
    line_alt,   = ax_alt.plot([], [], color="tab:green")
    line_vel,   = ax_vel.plot([], [], color="tab:purple")

    plt.tight_layout()
    return (fig, ax_bank, ax_err, ax_alt, ax_vel,
            line_lstm, line_apollo, line_err, line_alt, line_vel,
            line_err_x, line_err_y, line_err_z)


def update_live_plots(fig, axes_lines, step,
                      bank_lstm_hist, bank_apollo_hist,
                      error_hist, err_xyz_hist, alt_hist, vel_hist):
    (ax_bank, ax_err, ax_alt, ax_vel,
     line_lstm, line_apollo, line_err, line_alt, line_vel,
     line_err_x, line_err_y, line_err_z) = axes_lines

    t = np.arange(step)
    line_lstm.set_data(t, np.degrees(bank_lstm_hist[:step]))
    t_full = np.arange(len(bank_apollo_hist))
    line_apollo.set_data(t_full, np.degrees(bank_apollo_hist))

    # Per-axis position error overlay (convert m → km for readability)
    line_err_x.set_data(t, err_xyz_hist[:step, 0] * 1e-3)
    line_err_y.set_data(t, err_xyz_hist[:step, 1] * 1e-3)
    line_err_z.set_data(t, err_xyz_hist[:step, 2] * 1e-3)
...
def run_simulation():
    ...
    print(f"Target (ECEF)    : {target}")
        ...
        # --- Load reference Apollo bank angle trajectory ---
    APOLLO_NPY_PATH = "Single_Roll/roll_single.npy"  # <-- your file

    apollo_ref = np.load(APOLLO_NPY_PATH).squeeze()

    # Ensure it's 1D
    apollo_ref = apollo_ref.reshape(-1)

    # Convert if needed (assume radians already — change if degrees)
    # apollo_ref = np.radians(apollo_ref)

    ref_len = len(apollo_ref)
    print(f"Loaded Apollo reference trajectory: {ref_len} steps")

    # --- Load reference Apollo position error trajectory (err_single.npy) ---
    ERR_NPY_PATH = "Single_Roll/err_single.npy"      # shape (timesteps, 3)
    err_ref = np.load(ERR_NPY_PATH)                  # (T, 3)
    err_ref = err_ref.reshape(-1, 3)
    print(f"Loaded Apollo reference error trajectory: {err_ref.shape[0]} steps x 3")
    ...
    bank_apollo_hist = apollo_ref
    error_hist      = np.zeros(SIM_STEPS)
    err_xyz_hist    = np.zeros((SIM_STEPS, 3))   # per-axis error [x, y, z]
    ...
    (fig, ax_bank, ax_err, ax_alt, ax_vel,
     line_lstm, line_apollo, line_err, line_alt, line_vel,
     line_err_x, line_err_y, line_err_z) = init_live_plots(err_ref)
    ...
    for step in range(SIM_STEPS):
        ...
        # ---- Termination check — hard stop at 5250 steps ----
        if step >= 5250:
            print(f"\nReached 5250-step limit (live version hard stop).")
            final_step = step
            break
        ...
        bank_apollo_hist[step] = apollo_ref[step]
        error_hist[step]       = error_mag
        err_xyz_hist[step]     = error_vec          # (dis_x, dis_y, dis_z) this step
